...
export default {
  ...
  mounted() {
    ...
    this.$refs.coverPopup.$el.childNodes[1].childNodes[0].style.display =
      'none';
  },
  watch: {
  ...
  methods: {
      ...
      reader.addEventListener('load', () => {
        this.$store
          ...
          .then((compressedDataURL) => {
            console.log('dsad', compressedDataURL);
            this.stepTwoProps.imageURL = compressedDataURL;
          })
          .catch((e) => {
            console.log(e);
            this.$vs.notify({
    ...
    checkAspectRatio(event) {
      const inputFiles =
        event.target.files || event.dataTransfer.files || this.$refs.coverfile.files;
    ...
    uploadBrandLogo(event) {
      ...
      reader.onload = () => {
        ...
        this.$store
          ...
          .then((compressedDataURL) => {
            console.log('logoCUrl', compressedDataURL);
            document.getElementById('brand-logo-preview').style.backgroundImage = 'url(' + compressedDataURL + ')';
            this.stepTwoProps.logo = file ;
            this.stepTwoProps.logoUrl = compressedDataURL;
            const nameTruncerBrand = file.name;
            this.logoFileName = nameTruncerBrand.length > 24 ? nameTruncerBrand.slice(-24).concat('...') : nameTruncerBrand;
