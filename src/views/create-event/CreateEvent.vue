  ...
  methods: {
    ...
    async localStorageFiller() {
      ...
      if (localStorage.getItem('Step2')) {
        ...
        const response_logo = await fetch(this.stepTwoProps.logoUrl);
        const contentType_logo = response_logo.headers.get('content-type');
        const blob_logo = await response_logo.blob();
        const file_logo = new File([blob_logo], 'coverimage', { type: contentType });
        this.stepTwoProps.logo = file_logo ;
      }
    ...
    validateFormOne() {
      if (
        ...
        if (this.stepOneProps.invalidTimeError) {
          ...
          var fieldElement = document.getElementById('startHour'); 
          if (fieldElement) {
          ...
          if(this.stepOneProps.event_name === ''){
            var fieldElement = document.getElementById('eventName'); 
            if (fieldElement) {
              fieldElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
         } 
         if( this.stepOneProps.description === ''){
            var fieldElement = document.getElementById('description'); 
            if (fieldElement) {
              fieldElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
         }
    ...
    validateFormTwo() {
      if (this.stepTwoProps.cover_image === '') {
        ...
        var fieldElement = document.getElementById('upload-cover-container-step2'); 
        if (fieldElement) {
          fieldElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
        return false;
