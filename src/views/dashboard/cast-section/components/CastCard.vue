<template>
  ...
  <div v-else-if="isMobileView" class="child-options" :style="{ borderRight: '5px solid ' + getColor(index) }"
    ...
    <div class="inner-div2" v-if="expandedRoom !== null && expandedRoom !== index">
      <button class="k-btn" @click="togglePopup(index, cast.public_meeting_id, cast.invitee_list)">
        <img src="@/assets/images/dashboard/dots3.svg" class="h-7 p-2 text-white" alt="" />
      </button>
    </div>

    <div class="inner-div2" v-else-if="expandedRoom === index" :style="{ backgroundColor: getColor(index) }">
      ...
      <button class="k-btn" @click.stop @click="togglePopup(index, cast.public_meeting_id, cast.invitee_list)">
        <img src="@/assets/images/dashboard/dots3.svg" class="h-7 p-2 text-white" alt="" />
      </button>
      ...
      <div class="inner-child3">
        <div class="inner-child4">
          <button class="copy-button cop-btn border-none" @click.stop @click="toggleCopy(index)"
            v-if="expandedRoom === index" :style="{ backgroundColor: getColor(index) }">
          ...
          <button id="new-id" v-if="cast.is_running === 'true' && expandedRoom === index">
            live
          </button>
          ...
          <div id="copy-pop" @click.stop v-if="showCopy === index">
            ...
            <button @click="copy(cast.public_meeting_id, undefined)" id="copy-btn-3"
          ...
          <button class="copy-button border-none" @click.stop v-if="cast.is_running === 'false' && expandedRoom === index"
            ...
            <img :style="{ backgroundColor: getColor(index) }" src="@/assets/images/dashboard/record.svg"
              class="p-3 mr-5 bg-white bg-opacity-50" style="
                border: none;
                padding: 10px;
                background: rgba(255, 255, 255, 0.5);
                border-radius: 5px;
  ...
  <div v-else class="child-options">
        ...
        <div class="inner-child4">
          <div v-if="streamInfo[cast.public_meeting_id]">
            ...
            <vx-tooltip v-else text="Start Stream" position="bottom">
              <vs-button class="vs-cast-copy-button" @click="toggleStream(cast.public_meeting_id, 'start')">
    ...
    <ResheduleCastModal v-if="activeModal == 'postponeCastModal'" :closeModal="() => setCastModal('')"
      ...
      :allow_moderator_to_unmute_user="castsInfo[cast.public_meeting_id].details.allow_moderator_to_unmute_user
        " :auto_start_recording="castsInfo[cast.public_meeting_id].details.auto_start_recording
    " :description="castsInfo[cast.public_meeting_id].details.description"
      :cast_type="castsInfo[cast.public_meeting_id].details.cast_type" :collect_attendee_email="castsInfo[cast.public_meeting_id].details.collect_attendee_email
        " :private_otp="castsInfo[cast.public_meeting_id].details.otp_private"
      :timezone="castsInfo[cast.public_meeting_id].details.timezone"
      :logo="castsInfo[cast.public_meeting_id].details.logo"
      :cover_image="castsInfo[cast.public_meeting_id].details.cover_image"
      ...
      :welcome_text="castsInfo[cast.public_meeting_id].details.welcome_text"
      :banner_text="castsInfo[cast.public_meeting_id].details.banner_text"
      :guest_policy="castsInfo[cast.public_meeting_id].details.guest_policy" :moderator_only_text="castsInfo[cast.public_meeting_id].details.moderator_only_text
        " :logout_url="castsInfo[cast.public_meeting_id].details.logout_url"
      :is_streaming="castsInfo[cast.public_meeting_id].details.is_streaming"
      :public_stream="castsInfo[cast.public_meeting_id].details.public_stream"
      :bbb_stream_url="castsInfo[cast.public_meeting_id].details.bbb_stream_url"
      :record="castsInfo[cast.public_meeting_id].details.record"
      :password_auth="castsInfo[cast.public_meeting_id].details.password_auth" :end_when_no_moderator="castsInfo[cast.public_meeting_id].details.end_when_no_moderator
        " :mute_on_start="castsInfo[cast.public_meeting_id].details.mute_on_start" :webcam_only_for_moderator="castsInfo[cast.public_meeting_id].details.webcam_only_for_moderator
    " :disable_cam="castsInfo[cast.public_meeting_id].details.disable_cam"
      :disable_mic="castsInfo[cast.public_meeting_id].details.disable_mic"
      :lock_layout="castsInfo[cast.public_meeting_id].details.lock_layout"
      :viewer_mode="castsInfo[cast.public_meeting_id].details.viewer_mode"
