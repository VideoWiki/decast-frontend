<template>
    <div id="kk-cont">
        <div class="mm-cont">
            ...
            <!-- <h2>{{start_time }}</h2> -->
            <h3>Cast is live <img src="@/assets/images/dashboard/Live.svg" alt="" /></h3>
            <h4>{{ e_title }}</h4>
            ...
            <form :class="[this.public && !this.public_auth ? 'join-box' : 'lets-see']">
                <div>
                    ...
                    <div v-if="!public_auth">
                        <input class="verification-input text-xl" v-validate="'required'" name="Name"
                            placeholder="e.g John G. Miguel" autocomplete="off" v-model="name" />
                    </div>

                    <div class="flex flex-wrap my-3">
                        <button class="acc-btn flex-1 h-16" :disabled="!validateForm" @click.prevent="joinStreaming">Access
                            Cast</button>
                    </div>
                </div>
            </form>
