        ...
        <div class="parent-img">
            <div class="main-img">
                <div v-for="(invite, inviteIndex) in invites" :key="inviteIndex" class="images-container">
                    <img v-for="(image, imageIndex) in invite.images" :key="imageIndex" :src="image" alt=""
                        class="user-image" />
                    <button class="close-button" @click="removeUser(inviteIndex)">X</button>
                </div>
            </div>
...
import axios from '../../axios';
const API = 'URL';
const SAMPLE_LIST_URL = 'https://videowikistorage.blob.core.windows.net/room-db-backup/Sample_Invitee_List.xlsx';
export default {
    name: 'InviteCard',
    props: {
        showInvite: Boolean,
            ...
            invites: [
                {
                    ...
                    role: 'participant',
                    images: ['https://i.pinimg.com/564x/b8/51/78/b85178bffa0f26892173b37fe22fec1b.jpg'],
                },
                {
    ...
    methods: {
        ...
        removeUser(user, inviteIndex) {
            ...
            this.$store.dispatch('studio/deleteInvitee', payload)
                .then((res) => {
                ...
                .catch((err) => {
                    ...
                    this.$vs.notify({
                        title: 'Error',
                        text: err.response != null ? err.response.data.message : 'Try again !',
                        color: 'danger',
                    });
                });
        },
        ...
        submitSheet() {
            ...
            const sessionKey = 'rkry-gb38-9d45-853u';
            var formData = new FormData();
            formData.append(
                'file',
                file,
                // console.log(URL.createObjectURL(this.fileInput))
            );
            formData.append('cast_id', sessionKey);
            // const file = fileInput.files[0];
            // console.log(file);
            // const payload = {
            //     file,
            //     meetingID: sessionKey,
            //     type: this.selectedOption,
            // };
            // console.log(payload);
            axios.
